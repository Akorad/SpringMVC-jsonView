...
public class SecurityConfig {
        ...
        http
            .csrf(AbstractHttpConfigurer::disable)
                ...
                .authorizeHttpRequests(auth -> auth
                    .requestMatchers("/api/auth/**","/api/oauth2/**").permitAll() // Public endpoints
                ...
                .oauth2Login(oauth2 -> oauth2
                    .loginPage("/api/oauth2/login") // Custom login page URL
                        .userInfoEndpoint(userInfo -> userInfo
                            .userService(socialAppService) // Custom user service to fetch user details
                        )
                    .defaultSuccessUrl("/api/oauth2/user") // Redirect URL after successful login
                    .permitAll()
                )
    ...
    private LogoutSuccessHandler oidcLogoutSuccessHandler() {
        return (request, response, authentication) -> {
            auditService.logEvent(authentication.getName(), "LOGOUT", "User logged out successfully");
            response.sendRedirect("/api/oauth2/login");
        };
    }
}
